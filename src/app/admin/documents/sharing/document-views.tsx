// src/app/admin/documents/sharing/document-views.tsx
import { useState, useEffect } from "react"
import { 
  FolderIcon, FileText, Trash2,
  FileImage, FileSpreadsheet, FileJson, FileCode, FileArchive, Music, Video,
  Share2, Pencil, Download, Info, Globe, Users, Calendar, HardDrive, Loader2
} from "lucide-react"
import {
  ContextMenu,
  ContextMenuContent,
...
interface SharedGroupsResponse {
...
interface DocumentDetailResponse {
  ...
  updated_at: string
  share_with_all?: boolean
  shared_groups?: SharedGroupsResponse[]
  access_level: "editor" | "viewer"
  owner_name: string
}
...
const getFileConfig = (item: DocumentItem) => {
  ...
  if (["html", "css", "js", "ts", "py", "php"].includes(extension || "")) return { Icon: FileCode, text: "text-blue-600" }
...
function InfoDialog({ item, isOpen, onClose, formatSize }: { item: DocumentItem | null; isOpen: boolean; onClose: () => void; formatSize: (size: number | string) => string }) {
  ...
  useEffect(() => {
    ...
    const loadDataDetail = async (id: string | number) => {
      setLoading(true)
      try {
        const response = await sharingService.getSharedDocumentsById(id)
        if (isMounted && response?.data) {
          setDetail(response.data)
        }
      } catch (error) {
        if (isMounted) {
          console.error("Gagal memuat detail dokumen bersama:", error)
        }
      } finally {
  ...
  const displaySize = detail?.is_folder ? "Folder" : formatSize(detail?.file_size || item.file_size)
  const displayDate = detail?.updated_at || item.updated_at
  const displayIsShared = detail !== null ? detail.is_shared : item.is_shared
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[420px]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 py-2">
          <div className="flex flex-col items-center justify-center p-4 bg-gray-50 rounded-xl border border-dashed relative min-h-[120px]">
            {loading && (
            ...
            <span className="text-sm font-semibold text-gray-800 text-center max-w-[320px] break-all">{displayName}</span>
          </div>
          ...
          <div className="space-y-2.5 text-sm">
            ...
            <div className="flex items-center justify-between border-b pb-2">
              ...
              <span className="font-medium text-gray-900">
                {new Date(displayDate).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" })}
              </span>
            </div>
                ...
                {!displayIsShared ? (
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {/* 🛠️ FIX MASALAH: Ganti detail.group_names menjadi detail.shared_groups */}
                      {detail?.shared_groups && detail.shared_groups.length > 0 ? (
                        detail.shared_groups.map((group, i) => (
                          <span key={group.group_id || i} className="px-2 py-0.5 bg-blue-100 text-blue-700 border border-blue-200 rounded text-[11px] font-medium">
                            {group.group_name} (<span className="font-bold capitalize">{group.access_level}</span>)
                          </span>
                        ))
                      ) : loading ? (
                        <span className="text-gray-400 italic flex items-center gap-1">
                          <Loader2 className="w-3 h-3 animate-spin" /> Memuat daftar grup...
                        </span>
                      ) : (
                        <span className="text-gray-400 italic">Daftar grup pembaca tidak ditemukan</span>
                      )}
                    </div>
                  </div>
                )}
